...
const Home = () => {
	useEffect(() => {
		...
		var RENDERER = {
			...
			setParameters: function (strategy) {
				...
				this.$container = refHome.current;
				this.width = this.$container.offsetWidth;
				this.height = this.$container.offsetHeight;

				this.$canvas = refCanvas.current;

				refCanvas.current.width = this.width;
				refCanvas.current.height = this.height;
				...
				this.strategies = strategy.getStrategies(strategy.getStrategies);
				this.particles = [];
			},
			...
			changeAngle: function (event) {
				var offset = {
					...
					x = event.clientX - offset.left + this.$window.screenLeft,
					y = event.clientY - offset.top + this.$window.screenTop;
		...
		var STRATEGY = {
			...
			getStrategies: function (func) {
				var strategies = [];

				for (var i in this) {
					if (this[i] == func || typeof this[i] != 'function') {
						continue;
					}
	...
	return <section className="home" ref={refHome}>
